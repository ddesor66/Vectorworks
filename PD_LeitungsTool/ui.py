...
def _right_side_position(dialog, preferred_size=None):
    ...
    try:
        ...
        palette_width = max(280, min(420, int(max(1, right-left) * 0.22)))
...
def route_dialog(preferences, initial=None, source_count=0):
    ...
    labels = (
        ...
        (50, "Regelmäßiger Beschriftungstext:"), (52, "Beschriftungsabstand [m]:"),
        (58, "Schriftart / Größe [pt]:"))
    ...
    for item, value in defaults.items():
        vs.CreateEditText(dialog, item, str(value).replace(".", ","), 30 if item in (15, 21, 23, 51) else 14)
    ...
    previous = None
    for label, field in ((50, 51), (52, 53), (58, 59)):
        if previous is not None:
            vs.SetBelowItem(dialog, previous, label, 0, 5)
        vs.SetRightItem(dialog, label, field, 8, 0)
        previous = label
...
def height_chain_dialog(route):
    ...
    rows = [(line_index, point_index,
             "Leitung %d · Punkt %d · %.3f m" %
             (line_index + 1, point_index + 1,
              current["route_heights_m"][line_index][point_index]))
    ...
    def show():
        ...
        vs.SetItemText(dialog, 15, ("%.4f" % value).replace(".", ","))
...
def preferences_dialog(preferences):
    ...
    def handler(item, _data):
        if item == INIT:
            for index, value in enumerate(core.UTILITY_TYPES):
                vs.AddChoice(dialog, 12, value, index)
            for index, value in enumerate(current["materials"]):
                vs.AddChoice(dialog, 16, value, index)
            vs.SelectChoice(dialog, 12, list(core.UTILITY_TYPES).index(current["default_type"]), True)
            ...
            value = dict(current)
            value.update(
                default_type=core.UTILITY_TYPES[_choice(dialog, 12)],
                default_dn_mm=core.integer(_text(dialog, 14), "Standard-DN", 1, 10000),
                default_material=current["materials"][_choice(dialog, 16)],
                ...
                colors={utility_type: list(vs.ColorIndexToRGB(vs.GetColorChoice(dialog, control)))
                        for control, utility_type in zip((26, 27, 28, 29), core.UTILITY_TYPES)})
            result["value"] = value
        return item
    return result["value"] if _run(dialog, handler) == 1 else None
