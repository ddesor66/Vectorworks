...
def _partition_visibility_rows(rows, selected_indexes, remove_selected):
    """Return remaining and removed control-list rows without mutating input."""
    rows = tuple(rows or ())
    selected = {
        int(index) for index in (selected_indexes or ())
        if 0 <= int(index) < len(rows)
    }
    remove_indexes = (
        selected if remove_selected
        else set(range(len(rows))).difference(selected)
    )
    remaining = tuple(
        row for index, row in enumerate(rows) if index not in remove_indexes)
    removed = tuple(
        row for index, row in enumerate(rows) if index in remove_indexes)
...
def _choice(dialog, item_id, fallback=0):
    """Read a pull-down selection without silently losing the user's choice.

    Some Vectorworks installations return an unusable value from
    ``GetSelectedChoiceIndex`` although ``GetItemText`` already contains the
    selected label.  Prefer the label lookup and accept the scalar/tuple
    return shapes seen in different Python bridges.
    """
...
def choose_names(title, names, logo_path=None):
    I_OK, I_CANCEL = 1, 2
...
def choose_report_columns(selected_columns=None, logo_path=None):
    """Choose visible quantity columns with ordinary on/off check boxes."""

    I_OK, I_CANCEL = 1, 2
    I_TITLE, I_LOGO, I_HINT, I_ALL, I_CORE = range(10, 15)
    check_ids = tuple(range(100, 100 + len(HEADERS)))
    selected = set(normalize_visible_columns(selected_columns))
    result = {"values": None}
    dialog = vs.CreateLayout(
        _dialog_title("PD Massentabelle – Spalten"), True,
        "Übernehmen", "Abbrechen")
    vs.CreateStyledStatic(
        dialog, I_TITLE, "MASSENTABELLE  |  SICHTBARE SPALTEN", -1,
        TITLE_STYLE)
    try:
        vs.CreateImageControl2(dialog, I_LOGO, 76, 39, "")
    except Exception:
    ...
    vs.CreateStaticText(
        dialog, I_HINT,
        "Haken setzen, wenn die Spalte im Vectorworks-Arbeitsblatt und in "
        "der XLSX-Ausgabe erscheinen soll.", 80)
    for item_id, header in zip(check_ids, HEADERS):
        vs.CreateCheckBox(dialog, item_id, header)
    ...
    try:
        ...
        pass
    vs.SetBelowItem(dialog, I_TITLE, I_HINT, 0, 8)
    left_ids = check_ids[:10]
    right_ids = check_ids[10:]
    vs.SetBelowItem(dialog, I_HINT, left_ids[0], 0, 5)
    for previous, current in zip(left_ids, left_ids[1:]):
        vs.SetBelowItem(dialog, previous, current, 0, 2)
    vs.SetRightItem(dialog, left_ids[0], right_ids[0], 28, 0)
    for previous, current in zip(right_ids, right_ids[1:]):
        vs.SetBelowItem(dialog, previous, current, 0, 2)
    vs.SetBelowItem(dialog, left_ids[-1], I_ALL, 0, 7)
    vs.SetRightItem(dialog, I_ALL, I_CORE, 8, 0)

    def set_checked(headers):
        checked = set(headers)
        for item_id, header in zip(check_ids, HEADERS):
            vs.SetBooleanItem(dialog, item_id, header in checked)
    ...
    def handler(item, _data):
        if item == INIT_EVENT:
            ...
            set_checked(CORE_HEADERS)
        elif item == I_OK:
            values = tuple(
                header for item_id, header in zip(check_ids, HEADERS)
                if vs.GetBooleanItem(dialog, item_id))
            if not values:
                vw_adapter.alert(
...
def visibility_dialog(logo_path=None):
    ...
    (I_TITLE, I_LOGO, I_C_HEAD, I_C_ACTIVE, I_C_IN_L, I_C_IN,
     ...
     I_REMOVE, I_REMOVE_UNMARKED, I_UNDO_REMOVE, I_ACTION_L, I_ACTION,
     I_RESTORE, I_HINT) = range(10, 39)
    state = {
        "rows": [],
        "manual_classes": set(),
        "manual_layers": set(),
    ...
    # Start compact enough for notebook displays. The list expands together
    # with the resizable dialog through the edge bindings below.
    vs.CreateLB(dialog, I_LIST, 76, 4)
    vs.CreatePushButton(dialog, I_REMOVE, "Markierte Einträge aus Kontrollliste entfernen")
    vs.CreatePushButton(
        dialog, I_REMOVE_UNMARKED,
        "Nicht markierte Einträge aus Kontrollliste löschen")
    vs.CreatePushButton(
        dialog, I_UNDO_REMOVE, "Letzte Listenlöschung rückgängig")
    vs.CreateStaticText(dialog, I_ACTION_L, "Aktion:", -1)
    vs.CreatePullDownMenu(dialog, I_ACTION, 48)
    vs.CreateCheckBox(
        dialog, I_RESTORE,
        "Stattdessen: letzte Sichtbarkeit vollständig wiederherstellen")
    vs.CreateStaticText(
        dialog, I_HINT,
    ...
    vs.SetRightItem(dialog, I_REMOVE, I_REMOVE_UNMARKED, 4, 0)
    vs.SetRightItem(dialog, I_REMOVE_UNMARKED, I_UNDO_REMOVE, 4, 0)
    vs.SetBelowItem(dialog, I_REMOVE, I_ACTION_L, 0, 4)
    vs.SetRightItem(dialog, I_ACTION_L, I_ACTION, 4, 0)
    vs.SetRightItem(dialog, I_ACTION, I_RESTORE, 8, 0)
    vs.SetBelowItem(dialog, I_ACTION_L, I_HINT, 0, 4)

    # The control list absorbs resizing while all controls below it stay
    # attached to the lower dialog edge. This keeps the native confirmation
    # buttons reachable on compact screens and avoids overlaps after resizing.
    vs.SetEdgeBinding(dialog, I_LIST, True, True, True, True)
    for item_id in (
            I_REMOVE, I_REMOVE_UNMARKED, I_UNDO_REMOVE, I_ACTION_L,
            I_ACTION, I_RESTORE):
    ...
    def fill_control_list():
        _fill_lb(dialog, I_LIST, [
            (kind, name, "wird geändert") for kind, name in state["rows"]])

    def remember_list_state():
        state["list_history"].append((
            tuple(state["rows"]),
            frozenset(state["removed_classes"]),
            frozenset(state["removed_layers"]),
        ))
        del state["list_history"][:-20]

    def remove_control_rows(remove_selected):
        selected = set(_selected_lb_rows(dialog, I_LIST, len(state["rows"])))
        if not selected:
            vw_adapter.alert(
                "Mindestens einen Eintrag markieren. Bei der Funktion für "
                "nicht markierte Einträge bleiben die markierten Einträge erhalten.")
            return
        remaining, removed = _partition_visibility_rows(
            state["rows"], selected, remove_selected)
        if not removed:
            vw_adapter.alert("Es sind keine passenden Einträge zum Löschen vorhanden.")
            return
        remember_list_state()
        for kind, name in removed:
            ...
            target.add(name)
        state["rows"] = list(remaining)
        fill_control_list()

    def undo_control_row_removal():
        if not state["list_history"]:
            vw_adapter.alert("Es ist keine Listenlöschung zum Rückgängigmachen vorhanden.")
            return
        rows, removed_classes, removed_layers = state["list_history"].pop()
        state["rows"] = list(rows)
        state["removed_classes"] = set(removed_classes)
        state["removed_layers"] = set(removed_layers)
        fill_control_list()
    ...
    def handler(item, _data):
        if item == INIT_EVENT:
            ...
            remove_control_rows(True)
        elif item == I_REMOVE_UNMARKED:
            remove_control_rows(False)
        elif item == I_UNDO_REMOVE:
            undo_control_row_removal()
        elif item == I_OK:
            if vs.GetBooleanItem(dialog, I_RESTORE):
                state["result"] = {"action": "restore"}
                return item
...
def quantity_update_choice_dialog(worksheet_name, logo_path=None):
    """Offer a true plug-in update before opening the full quantity dialog."""
    ...
    state = {"result": None}
    dialog = vs.CreateLayout(
        _dialog_title("PD Massentabelle aktualisieren"), True,
        "Weiter", "Abbrechen")
    vs.CreateStyledStatic(
        dialog, I_TITLE, "MASSENTABELLE  |  AKTUALISIEREN", -1,
        TITLE_STYLE)
    try:
        vs.CreateImageControl2(dialog, I_LOGO, 76, 39, "")
    except Exception:
    ...
    vs.CreateStaticText(
        dialog, I_HINT,
        "Für dieses Dokument ist bereits die Massentabelle „%s“ vorhanden."
        % str(worksheet_name or ""), 76)
    vs.CreateStaticText(dialog, I_ACTION_L, "Aktion:", -1)
    ...
    def handler(item, _data):
        if item == INIT_EVENT:
            ...
            vs.AddChoice(
                dialog, I_ACTION,
                "Vorhandene Massentabelle jetzt aktualisieren", 0)
            vs.AddChoice(
                dialog, I_ACTION,
                "Einstellungen prüfen oder Auswahl ändern", 1)
        elif item == I_OK:
...
def quantity_dialog(
    ...
    (I_TITLE, I_LOGO, I_FILTER_HEAD, I_C_ACTIVE, I_C_L, I_C, I_C_PICK,
     ...
     I_COLUMNS, I_COLUMNS_SUMMARY, I_HINT) = range(10, 47)
    ...
    dialog = vs.CreateResizableLayout(
        ...
        "Massentabelle erstellen/aktualisieren", "Abbrechen", True, True)
    vs.CreateStyledStatic(
        dialog, I_TITLE,
    ...
    vs.CreateEditText(dialog, I_C, "", 34)
    ...
    vs.CreateEditText(dialog, I_L, "", 34)
    ...
    # Keep the initial dialog usable on compact displays. The list grows with
    # the dialog through the edge bindings below, so larger screens still gain
    # additional visible rows.
    vs.CreateLB(dialog, I_LIST, 92, 4)
    ...
    vs.CreateStaticText(
        dialog, I_HINT,
        "Alle auswertbaren Längen, Umfänge und Flächen werden nach Klasse, "
        "Ebene und Elementtyp erfasst. Symbole und Gruppen werden je Typ "
        "getrennt aufgeführt. Parallelabstände lassen sich gemeinsam oder je "
        "erkannter Tabellenzeile festlegen. Die Kontrollliste zeigt vor der "
        "Ausgabe exakt, welche Elemente ausgewertet werden.", 104)
    ...
    try:
        ...
        pass
    vs.SetBelowItem(dialog, I_TITLE, I_FILTER_HEAD, 0, 4)
    ...
    vs.SetBelowItem(dialog, I_REFRESH, I_LIST_HEAD, 0, 4)
    vs.SetBelowItem(dialog, I_LIST_HEAD, I_LIST, 0, 2)
    vs.SetBelowItem(dialog, I_LIST, I_REMOVE, 0, 2)
    vs.SetBelowItem(dialog, I_REMOVE, I_GROUP_HEAD, 0, 4)
    ...
    vs.SetBelowItem(dialog, I_PAR_VALUE_L, I_OUTPUT_HEAD, 0, 4)
    ...
    vs.SetBelowItem(dialog, I_XLSX, I_COLUMNS, 0, 3)
    vs.SetRightItem(dialog, I_COLUMNS, I_COLUMNS_SUMMARY, 8, 0)
    vs.SetBelowItem(dialog, I_COLUMNS, I_HINT, 0, 3)

    # The list absorbs vertical and horizontal resizing. Everything below it
    # follows the lower dialog edge, preventing overlaps while keeping the
    # action buttons reachable when the window is made smaller.
    vs.SetEdgeBinding(dialog, I_LIST, True, True, True, True)
    for item_id in (
            I_REMOVE, I_GROUP_HEAD, I_GROUP_L, I_GROUP, I_ASSIGN, I_CLEAR,
            I_PAR_HEAD, I_PAR_MODE_L, I_PAR_MODE, I_PAR_VALUE_L, I_PAR_VALUE,
            I_PAR_APPLY, I_OUTPUT_HEAD, I_EXACT, I_WS, I_XLSX, I_RESULTS,
            I_COLUMNS, I_COLUMNS_SUMMARY):
        vs.SetEdgeBinding(dialog, item_id, True, False, False, True)
    vs.SetEdgeBinding(dialog, I_HINT, True, True, False, True)

    def make_selection():
        class_filter = DimensionFilter.from_expressions(
            bool(vs.GetBooleanItem(dialog, I_C_ACTIVE)),
            _patterns(vs.GetItemText(dialog, I_C)),
            explicit_includes=state["manual_classes"])
        layer_filter = DimensionFilter.from_expressions(
            bool(vs.GetBooleanItem(dialog, I_L_ACTIVE)),
            _patterns(vs.GetItemText(dialog, I_L)),
            explicit_includes=state["manual_layers"])
    ...
    def handler(item, _data):
        if item == INIT_EVENT:
            ...
            if values is not None:
                if vs.GetItemText(dialog, I_C).strip() == "*":
                    vs.SetItemText(dialog, I_C, "")
                state["manual_classes"].update(values)
            ...
            if values is not None:
                if vs.GetItemText(dialog, I_L).strip() == "*":
                    vs.SetItemText(dialog, I_L, "")
                state["manual_layers"].update(values)
                state["removed_keys"] = set(
                    key for key in state["removed_keys"]
...
def home_dialog(logo_path=None):
    I_OK, I_CANCEL = 1, 2
    (I_TITLE, I_LOGO, I_HEAD, I_FILTER, I_RESTORE,
     I_REDO, I_HINT) = range(10, 17)
    state = {"mode": None}
    dialog = vs.CreateLayout(
        _dialog_title("PD Sichtbarkeiten"), True,
        "Werkzeug öffnen", "Abbrechen")
    vs.CreateStyledStatic(
        dialog, I_TITLE, "PD SICHTBARKEITEN", -1, TITLE_STYLE)
    try:
        vs.CreateImageControl2(dialog, I_LOGO, 90, 47, "")
    except Exception:
        pass
    vs.CreateStyledStatic(dialog, I_HEAD, "WERKZEUG AUSWÄHLEN", -1, SECTION_STYLE)
    vs.CreateRadioButton(dialog, I_FILTER, "Klassen und Ebenen filtern")
    vs.CreateRadioButton(
        dialog, I_RESTORE, "Letzte Sichtbarkeit vollständig wiederherstellen")
    vs.CreateRadioButton(
        dialog, I_REDO, "Wiederhergestellte Sichtbarkeit erneut anwenden")
    vs.CreateStaticText(
        dialog, I_HINT,
        "Klassen- und Ebenensichtbarkeiten werden vor jeder Änderung vollständig "
        "gesichert. Bis zu drei Schritte können zurück- und erneut angewendet "
        "werden.", 76)
    vs.SetFirstLayoutItem(dialog, I_TITLE)
    try:
        vs.SetRightItem(dialog, I_TITLE, I_LOGO, 16, 0)
    ...
    vs.SetBelowItem(dialog, I_HEAD, I_FILTER, 0, 3)
    vs.SetBelowItem(dialog, I_FILTER, I_RESTORE, 0, 3)
    vs.SetBelowItem(dialog, I_RESTORE, I_REDO, 0, 3)
    vs.SetBelowItem(dialog, I_REDO, I_HINT, 0, 8)

    def handler(item, _data):
        if item == INIT_EVENT:
            _set_logo(dialog, I_LOGO, logo_path)
            vs.SetBooleanItem(dialog, I_FILTER, True)
        elif item == I_OK:
            if vs.GetBooleanItem(dialog, I_RESTORE):
                state["mode"] = "restore"
            elif vs.GetBooleanItem(dialog, I_REDO):
                state["mode"] = "redo"
            else:
