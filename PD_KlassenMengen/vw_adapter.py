...
def _refresh_visibility_display(target_layer, layer_visibility):
    """Force Vectorworks to rebuild the class/layer display cache.

    ``ReDrawAll`` alone can leave the drawing visually stale after several
    class and layer visibility changes in one Python command.  A real layer
    activation invalidates that cache in Vectorworks 2026.  Prefer a visible
    alternate layer so no saved visibility value is changed, then return to
    the saved active layer.  A one-layer document falls back to reapplying its
    current zoom factor, which invalidates the view without moving its centre.
    """
    target_layer = str(target_layer or "")
    candidates = sorted(
        str(name) for name, visibility in (layer_visibility or {}).items()
        if str(name) != target_layer and int(visibility) != VIS_HIDDEN
    )
    for candidate in candidates:
        if _activate_layer(candidate):
            if target_layer:
                _activate_layer(target_layer)
            redraw()
            return "layer"

    zoom = _call("GetZoom", default=None)
    try:
        zoom_percent = int(round(float(zoom)))
    except (TypeError, ValueError, OverflowError):
        zoom_percent = 0
    if zoom_percent > 0:
        _call("SetZoom", zoom_percent)
    redraw()
    return "zoom"


def unhide_all_objects():
    """Clear Vectorworks' object-level hidden state and redraw the document."""
    _call("UnHideObjects")
    redraw()
...
def _set_viewport_visibility(viewport, classes, layers):
    ...
    # The dirty flag alone does not repaint a sheet-layer viewport.  Without
    # UpdateVP, restored overrides remain visually stale until Vectorworks
    # performs a later manual viewport update.
    _call("UpdateVP", viewport)
...
def visibility_values_equal(first, second):
    """Compare the visible result while deliberately ignoring active targets.

    Merely activating a class or layer (for example while locating a quantity
    result) must not consume one of the three visibility restore steps.
    """
    first = first or {}
    second = second or {}

    def normalized_map(snapshot, key):
        return dict(
            (str(name), int(value))
            for name, value in (snapshot.get(key) or {}).items())

    if normalized_map(first, "classes") != normalized_map(second, "classes"):
        return False
    if normalized_map(first, "layers") != normalized_map(second, "layers"):
        return False
    if int(first.get("class_options", 5)) != int(second.get("class_options", 5)):
        return False
    if int(first.get("layer_options", 5)) != int(second.get("layer_options", 5)):
        return False
    first_viewport = first.get("viewport")
    second_viewport = second.get("viewport")
    if bool(first_viewport) != bool(second_viewport):
        return False
    if first_viewport:
        if str(first_viewport.get("uuid") or "") != str(
                second_viewport.get("uuid") or ""):
            return False
        if normalized_map(first_viewport, "classes") != normalized_map(
                second_viewport, "classes"):
            return False
        if normalized_map(first_viewport, "layers") != normalized_map(
                second_viewport, "layers"):
            return False
    return True
...
def apply_visibility_snapshot(snapshot, unhide_objects=False):
    available_classes = set(class_names())
    ...
    if unhide_objects:
        # Vectorworks stores an additional visibility bit on objects hidden by
        # Hide, HideSelectedObjects or ShowOnlySelected.  Restoring the class
        # and layer matrices does not clear that bit, which leaves an object
        # snappable but visually absent.  UnHideObjects is the documented VW
        # 2026 command for resetting precisely that object-level state.
        _call("UnHideObjects")
    # Vectorworks 2026 may keep the old display list until the user changes
    # layers manually.  Reproduce that refresh internally so the restored
    # state is visible after this one command invocation.
    _refresh_visibility_display(
        target_layer, snapshot.get("layers") or {})
    mismatches = visibility_snapshot_mismatches(snapshot)
    if mismatches:
        raise RuntimeError(
            "Der vorherige Sichtbarkeitszustand konnte nicht vollständig "
            "wiederhergestellt werden:\n- " + "\n- ".join(mismatches[:12]))


def visibility_snapshot_mismatches(snapshot):
    """Return exact, user-relevant differences from a saved snapshot.

    This intentionally validates the stored status, the active class/layer,
    the global visibility options and any selected viewport overrides.  The
    caller only consumes the undo step after this check succeeds.
    """
    mismatches = []
    available_classes = set(class_names())
    for name, expected in (snapshot.get("classes") or {}).items():
        if name not in available_classes:
            continue
        actual = int(_call("GetCVis", name, default=VIS_VISIBLE))
        if actual != int(expected):
            mismatches.append(
                "Klasse %s: %s statt %s" % (name, actual, int(expected)))

    records = dict((record["name"], record) for record in layer_records())
    for name, expected in (snapshot.get("layers") or {}).items():
        record = records.get(name)
        if record is None:
            continue
        actual = int(_call("GetLVis", record["handle"], default=VIS_VISIBLE))
        if actual != int(expected):
            mismatches.append(
                "Ebene %s: %s statt %s" % (name, actual, int(expected)))

    expected_class = str(snapshot.get("active_class") or "")
    if expected_class in available_classes and active_class_name() != expected_class:
        mismatches.append("Aktive Klasse ist nicht " + expected_class)
    expected_layer = str(snapshot.get("active_layer") or "")
    if (expected_layer in set(all_layer_names()) and
            active_layer_name() != expected_layer):
        mismatches.append("Aktive Ebene ist nicht " + expected_layer)
    if int(_call("GetClassOptions", default=5) or 5) != int(
            snapshot.get("class_options", 5)):
        mismatches.append("Klassendarstellung stimmt nicht überein")
    if int(_call("GetLayerOptions", default=5) or 5) != int(
            snapshot.get("layer_options", 5)):
        mismatches.append("Ebenendarstellung stimmt nicht überein")
    ...
    if isinstance(viewport_state, dict):
        ...
        if (viewport and
                int(_call("GetTypeN", viewport, default=0) or 0) == TYPE_VIEWPORT):
            actual = _viewport_visibility_snapshot(viewport)
            for name, expected in (viewport_state.get("classes") or {}).items():
                if (name in actual["classes"] and
                        int(actual["classes"][name]) != int(expected)):
                    mismatches.append("Ansichtsbereich-Klasse " + name)
            for name, expected in (viewport_state.get("layers") or {}).items():
                if (name in actual["layers"] and
                        int(actual["layers"][name]) != int(expected)):
                    mismatches.append("Ansichtsbereich-Ebene " + name)
    return mismatches
