...
from .report_columns import normalize_visible_columns
...
def _state_store():
    legacy_path = os.path.join(_module_directory(), "PD_KM_Status.json")
    user_data = str(vw_adapter._call(
        "GetFolderPath", -12, default="") or "").strip()
    if not user_data:
        return StateStore(legacy_path)
    stable_path = os.path.join(
        user_data, "PD_ToolsPD", "PD_KM_Status.json")
    if not os.path.isfile(stable_path) and os.path.isfile(legacy_path):
        try:
            directory = os.path.dirname(stable_path)
            if not os.path.isdir(directory):
                os.makedirs(directory)
            shutil.copy2(legacy_path, stable_path)
        except OSError:
            return StateStore(legacy_path)
    return StateStore(stable_path)


def _product_database():
    """Return the legacy product-only database used for one-time migration."""
    return ProductDatabase(os.path.join(
        _module_directory(), "PD_KM_Massentabelle.sqlite3"))


def _mass_database():
    return MassDatabase(os.path.join(_module_directory(), DATABASE_FILENAME))
...
def run_visibility():
    ...
    if settings is None:
        return
    if settings.get("action") == "restore":
        run_visibility_restore(False)
        return
    ...
    try:
        ...
        if not vw_adapter.visibility_values_equal(snapshot, after):
    ...
    vw_adapter.info(
        ...
        "„Letzte Sichtbarkeit wiederherstellen“ (maximal 3 Schritte).")
...
def run_visibility_restore(redo=False):
    ...
    if not redo:
        # Object-level hiding is not represented by GetCVis/GetLVis and
        # therefore cannot participate in visibility_values_equal().  Reset
        # it before equal class/layer snapshots are discarded.  This also
        # repairs documents whose useful history was consumed by an older
        # release while the objects stayed snappable but invisible.
        _name_undo_event("PD Sichtbarkeit wiederherstellen")
        vw_adapter.unhide_all_objects()
        # Older versions recorded entries when only the active layer changed.
        # Skip those stale no-op entries so one click reaches the preceding
        # actual class/layer visibility state.
        while (snapshot is not None and
               vw_adapter.visibility_values_equal(snapshot, current)):
            store.discard_visibility_undo(
                key, snapshot, vw_adapter.now_timestamp())
            snapshot = store.peek_visibility_undo(key)
    if snapshot is None:
        if redo:
            vw_adapter.alert(
            ...
            vw_adapter.info(
                "Objekt-Ausblendungen wurden zurückgesetzt. Es ist kein "
                "weiterer Klassen-/Ebenenschritt vorhanden.")
        return
    if redo:
        _name_undo_event("PD Sichtbarkeit " + verb)
    try:
        # UnHideObjects must run again after the class/layer matrix has been
        # restored.  Vectorworks may leave the hidden bit untouched when the
        # object's class is still invisible during the first reset, which
        # otherwise makes a second restore command necessary.
        vw_adapter.apply_visibility_snapshot(
            snapshot, unhide_objects=not redo)
        if redo:
...
def _reveal_quantity_source(source_key, object_ids=()):
    ...
    try:
        ...
        vw_adapter.redraw()
        after = vw_adapter.capture_visibility()
        if vw_adapter.visibility_values_equal(after, before):
            return False
...
def run_quantities():
    ...
    previous_worksheet_name = str(
        previous_payload.get("worksheet_name") or "")
...
def guarded_main(mode="home"):
    try:
        ...
        log_path = os.path.join(_module_directory(), "PD_KM_Fehler.log")
        try:
        ...
        vw_adapter.alert(message, "PD Sichtbarkeiten")
