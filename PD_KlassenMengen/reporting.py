...
def replace_vectorworks_worksheet(
        ...
        show=True, visible_columns=None):
...
def create_vectorworks_worksheet(
    ...
    audit_values = _audit_values(analysis)
    total_column_count = max(len(visible_columns), len(AUDIT_HEADERS))
    total_rows = 4 + len(records) + len(audit_values)
    ws = _ws_call(
...
def _populate_vectorworks_worksheet(
    ...
    total_column_count = max(main_column_count, len(AUDIT_HEADERS))
    records = report_records(rows, group_titles, catalog)
    audit_values = _audit_values(analysis)
    ...
    try:
        ...
        try:
            ...
            # Leave row_number empty as a visual separator, then append the
            # complete duplicate-analysis audit to the native worksheet.
