...
def normalize_visible_columns(values=None):
    """Return known columns once while preserving the requested order.
    ...
    The stored order is part of the report definition and is therefore kept
    unchanged for the Vectorworks worksheet, XLSX export, and later updates.
    """
    ...
    for header in requested:
        if header in known and header not in seen:
...
def visible_column_indices(values=None):
    index_by_header = dict(
        (header, index) for index, header in enumerate(HEADERS))
    return tuple(
        index_by_header[header]
        for header in normalize_visible_columns(values))
