...
4. `Vorschau` zeigt verwendbare, ausgeschlossene, problematische und nicht unterstützte Objekte. Gleiche XY-Lagen mit widersprüchlicher Höhe und identische Geometrien werden protokolliert, aber im strikten Markierungsmodus vollständig übernommen. Nur ausdrücklich eingetragene Klassen-/Ebenenfilter, Objekte außerhalb einer ausdrücklich aktivierten Begrenzung und technisch nicht lesbare Objekte werden übersprungen; sie brechen die Ausgabe der übrigen gültigen Daten nicht ab.
   ...
   Der Auswahlmodus führt das Vectorworks-Auswahlkriterium, die ausgewählte Objektkette und die tiefe Auswahliteration zusammen. Zusätzlich wird jede native Ebenen- und Objektliste direkt über `FLayer`, `FInLayer` und `NextObj` durchlaufen. Dieser Weg umgeht die in großen DWG-Beständen beobachteten Callback-Grenzen, durch die gerade 2D-Texte und Linien fehlten. Stimmt die native Auswahlzahl nicht mit den gelesenen Handles überein, wird die vollständige direkte Objektliste der betroffenen Ebene verarbeitet. Gruppen werden anschließend rekursiv aufgelöst. Der Prüfdialog weist die eingelesenen Vectorworks-Objekttypen separat aus; der Abschluss nennt ausdrücklich die Zahl der aus Texten und Linien erzeugten Quellen. Die optionale Ebenenprüfung liest unabhängig davon die vollständige aktive Ebene. Identische Dubletten erscheinen als eine kompakte Summenzeile statt als UUID-Liste.
5. Nach Bestätigung werden DGM-fähige 3D-Punkte und Bruchkanten auf einer neuen Quellenebene angelegt und markiert. Zusätzlich entsteht eine eigene Kontrollebene mit sichtbaren 1:1-Kopien aller tatsächlich erfassten Text- und Linienobjekte. Diese Kontrollkopien bleiben unmarkiert und gelangen deshalb nicht in den nativen DGM-Befehl. Originale werden weder geändert noch gelöscht.
