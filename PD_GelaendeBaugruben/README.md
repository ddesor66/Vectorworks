...
1. Es werden ausnahmslos nur die aktuell markierten, von Vectorworks im 3D-Raum lokalisierbaren Objekte geprüft. Unmarkierte Objekte derselben, einer aktiven oder einer anderen Ebene werden nie automatisch ergänzt. Unterstützt sind 3D-Punkte, tatsächlich in 3D platzierte Texte, Vermessungspunkt-Plug-in-Objekte (auch gebräuchliche fremdsprachige Bezeichnungen), Symbolpunkte, 2D-/3D-Linien, 3D-Polygone/Bruchkanten, NURBS-Kurven, Polygone, Polylinien, abgetastete Bögen, Inhalte ausdrücklich markierter Gruppen und sämtliche Eckpunkte von Meshes mit gültiger Höhe. Weitere räumlich lokalisierbare Fremdtypen werden mindestens als 3D-Stützpunkt an ihrem Vectorworks-3D-Mittelpunkt übernommen.
...
4. `Vorschau` zeigt verwendbare, ausgeschlossene, problematische und nicht unterstützte Objekte. Gleiche XY-Lagen mit widersprüchlicher Höhe und identische Geometrien werden protokolliert, aber im strikten Markierungsmodus vollständig übernommen. Nur ausdrücklich eingetragene Klassen-/Ebenenfilter, Objekte außerhalb einer ausdrücklich aktivierten Begrenzung und technisch nicht lesbare Objekte werden übersprungen; sie brechen die Ausgabe der übrigen gültigen Daten nicht ab.
   ...
   Der Auswahlmodus führt das Vectorworks-Auswahlkriterium, die ausgewählte Objektkette und die tiefe Auswahliteration zusammen. Zusätzlich werden die nativen Ebenen- und Objektlisten direkt über `FLayer`, `FInLayer` und `NextObj` durchlaufen; aus diesem vollständigen Durchlauf werden jedoch ausschließlich Handles übernommen, für die Vectorworks selbst `Selected` meldet. Dieser Weg umgeht die in großen DWG-Beständen beobachteten Callback-Grenzen, durch die gerade markierte 2D-Texte und Linien fehlten, ohne benachbarte unmarkierte Objekte einzubeziehen. Gruppen werden nur dann rekursiv aufgelöst, wenn die Gruppe selbst markiert ist. Der Prüfdialog weist die eingelesenen Vectorworks-Objekttypen separat aus; der Abschluss nennt ausdrücklich die Zahl der aus Texten und Linien erzeugten Quellen. Identische Dubletten erscheinen als eine kompakte Summenzeile statt als UUID-Liste.
5. Nach Bestätigung werden DGM-fähige 3D-Punkte und Bruchkanten auf einer neuen, eindeutig benannten Quellenebene angelegt und markiert. Zusätzlich entsteht eine eigene, ebenfalls eindeutig benannte Kontrollebene mit sichtbaren 1:1-Kopien aller tatsächlich erfassten Text- und Linienobjekte. Diese Kontrollkopien bleiben unmarkiert und gelangen deshalb nicht in den nativen DGM-Befehl. Originale und bereits bestehende DGM werden weder geändert noch gelöscht. Für eine weitere Teilfläche wird eine neue Auswahl markiert und Schritt 1 erneut ausgeführt; bei gleichen Namensvorgaben erhalten Quellenebene, Kontrollebene und DGM automatisch fortlaufende Zusätze (`-2`, `-3`, …).
6. Anschließend startet das Modul automatisch den nativen Vectorworks-Befehl `Landschaft > Geländemodell > Geländemodell aus Ausgangsdaten`. Nach Bestätigung des nativen Dialogs wird das neu entstandene DGM sicher erkannt, benannt, seiner vorgesehenen Klasse zugewiesen, sichtbar geschaltet, einzeln markiert und nach abgeschlossenem PIO-Aufbau in das Zeichenfenster eingepasst. Wird der native Dialog abgebrochen, bleiben die geprüften Quellen erhalten.

3D-Polygone, NURBS-Kurven und Bruchkanten bleiben mit ihren individuellen Scheitelhöhen erhalten. Planare 3D-Texte liefern einen Stützpunkt an ihrem Einfügepunkt mit der tatsächlichen Z-Höhe ihrer Vectorworks-Objektmatrix; der für importierte Texte unzuverlässige 3D-Begrenzungsrahmen wird dafür nicht verwendet. Nur wenn keine räumliche Objektmatrix oder abweichende 3D-Lage lesbar ist und der Text genau einen eindeutigen Zahlenwert enthält (zum Beispiel `102.65` oder `H=102,65`), wird dieser Wert ersatzweise als Höhe in Metern verwendet. Texte mit mehreren Zahlen bleiben bewusst bei ihrer tatsächlichen Objekthöhe. Kurven werden nur mit der gewählten Sehnentoleranz abgetastet. Gruppen werden rekursiv gelesen, ohne die Originale aufzulösen. Meshes werden nicht geometrisch verändert; ihre vorhandenen Eckpunkte werden als einzelne 3D-Quellpunkte übernommen. Punktwolken bleiben mangels verifizierter, verlustfreier Python-Konvertierung ausdrücklich ausgeschlossen.

Symbolinstanzen, Volumenkörper und weitere konvertierbare räumliche Fremdtypen werden über eine temporäre Kopie in ihre vollständigen 3D-Polygone aufgelöst; die Kopie wird anschließend gelöscht. Dasselbe native Kopierverfahren wird bei importierten Linien verwendet, weil `GetSegPt*` innerhalb derselben DWG abwechselnd Dokument-XY oder einen zweiten georeferenzierten Koordinatenraum liefern kann. Text-XY stammt aus `HCenter`, die Höhe aus der tatsächlichen Objektmatrix; die in Vectorworks 2026 nicht vorhandene Funktion `GetTextOrigin` wird nicht mehr verwendet.

Die DGM-Quellen werden relativ zum internen Vectorworks-Nullpunkt angelegt. Dadurch trianguliert Vectorworks auch bei Vermessungskoordinaten mit mehreren Millionen Metern ohne verzerrte, kilometerweite Konturen; die Dokument-Georeferenz und die in der Objekt-Info-Palette angezeigten Weltkoordinaten bleiben unverändert. Die erzeugte Quellenebene bleibt nach erfolgreicher Ausgabe aktiv. Punkte und Bruchkanten erhalten zusätzlich eine gut sichtbare grüne Objektfarbe. Die getrennte Kontrollebene enthält die originalgetreuen Texte in Magenta und Linien in Orange, sodass die grafische Übernahme unmittelbar geprüft werden kann. Da Vectorworks beim Duplizieren den Auswahlstatus übernehmen kann und `DSelectAll` von den Ebenenoptionen abhängt, wird vor der DGM-Auswahl jedes Dokumentobjekt über die nativen Ebenenlisten ausdrücklich abgewählt. Danach markiert das Modul ausschließlich die normalisierten 3D-Punkte und 3D-Polygone der DGM-Quellenebene. Vectorworks zählt anschließend diese Geometrien und zusätzlich die dokumentweit markierten Objekte; bei jeder Abweichung oder einer markierten Kontrollkopie wird die Ausgabe zurückgerollt und als Fehler gemeldet. Nach dem nativen Erzeugungsdialog wird nicht anhand eines möglicherweise mehrfach vergebenen Namens geraten: Das Modul vergleicht die DGM-Objekt-IDs vor und nach dem Befehl und bearbeitet ausschließlich das neu hinzugekommene Modell.
...
- Der einzige Erzeugungsaufruf verwendet den geprüften universellen Vectorworks-Arbeitsbereichsnamen `DTM6 Menu`; sichtbare deutsche Menütexte werden nicht als technische Kennung verwendet.

## Technische API-Grenze

Die offizielle Vectorworks-2026-Python-Referenz stellt geprüfte Funktionen zum Finden und Abfragen bestehender nativer DGM bereit (`DTM6_GetDTMObject`, `DTM6_IsDTM6Object`, `DTM6_IsObjectReady`, `DTM6_GetZatXY`). In der vorhandenen offiziellen `vs.py` ist dagegen keine direkte Python-Funktion dokumentiert, die aus beliebigen Quellen ein neues natives DGM erzeugt, einen Modifikator sicher einem bestimmten DGM zuordnet, die Modifikatorebene einschaltet, das DGM garantiert aktualisiert oder native Abtrag-/Auftragswerte ausliest. Die DGM-Erzeugung wird deshalb über den nativen, modalen Vectorworks-Arbeitsbereichsbefehl gestartet; Triangulation und Einstellungen bleiben vollständig bei Vectorworks.

Darum trennt das Modul strikt:

- automatisierbar: Quellprüfung/-kopie, Start des nativen DGM-Erzeugungsdialogs, sichere Erkennung/Anzeige des neu erzeugten DGM, DGM-Abfrage, Sollkopien, numerischer Vergleich, Planausgabe, Tabelle und einfacher Pad-Attributversuch;
- nativer Benutzerschritt: Einstellungen im automatisch geöffneten DGM-Dialog bestätigen sowie spätere Modifikatorebenen prüfen und das DGM aktualisieren;
