...
import os
import vs
p = os.path.join(str(vs.GetFolderPath(-2) or ''), 'PD_Planpruefung', 'PD_Beschriftung.py')
if not os.path.isfile(p):
    found, candidate = vs.FindFileInPluginFolder('PD_Beschriftung.py')
    p = str(candidate or '') if found else ''
    if p and os.path.isdir(p):
        p = os.path.join(p, 'PD_Beschriftung.py')
